...
def applyTransforms(arr, transforms):
    ...
    if 'shearX' in transforms and 'shearY' in transforms:
        arr = shear(arr,transforms["shearX"],transforms["shearY"])
    ...
    if 'brightness' in transforms and 'contrast' in transforms:
        arr = brightness(arr,transforms["contrast"],transforms["brightness"])
    
    if 'blur' in transforms:
        arr = blur(arr,transforms["blur"])
    
    if 'noise' in transforms:
        arr = noise(arr,transforms["noise"])
    
    if 'padX' in transforms and 'padY' in transforms:
        arr = padding(arr,transforms["padX"],transforms["padY"])

    if 'gamma' in transforms:
        arr = gamma(arr,transforms["gamma"])
    
    if 'hist_eq' in transforms and transforms['hist_eq']:
        arr = histEqual(arr)
    
    if 'clahe' in transforms:
        arr = Clahe(arr,transforms["clahe"])
    
    if "lens_dis" in transforms and transforms["lens_dis"]:
        arr = LensDistortion(arr)
    return arr
